...
my $input_file = 'invoicePt1.tsv';
my $output_file = 'updatedInvoiceWithFields.csv';
my $exceptions_file = 'exceptionsInvoiceFirstStep.csv';
        ...
        if ($row->{$opt} =~ /^([^:]+):\s*(.*)/) {
           ...
           if (grep { lc $_ eq lc $first_part } @extendFields) {
                ...
                foreach my $part (@multiPart) {
                    process_option($row, $part,\$is_exception);
...
sub process_option {
    my ($row, $columnOption,$is_exception) = @_;
