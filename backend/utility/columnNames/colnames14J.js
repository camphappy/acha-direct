...
Includes rep and customername
        ...
        1           2         3            4         5           6             7             8
        QTY         PRODUCTS  OLD CODE     OPTION1   OPTION2     DESCRIPTION   PRICE EACH    AMOUNT
...
const scriptname = "This script: colnames14J.js";
...
const logFilePath = `./logcolnames14J_${datetime}.txt`;
const dataFilePath = `./colnames14J_${datetime}.tsv`;
...
async function main() {
    ...
    for (let i = 0; i < totalElements; i++) {
        ...
        if (!worksheet) {
            errorOut("Fail: Invoice worksheet does not exist");
            await fs.move(sourcename, relocdir);
            continue;
        }
        ...
        for (let rowIndex = dataRowsStartIndex; rowIndex <= dataRowsEndIndex; rowIndex++) {
            ...
            let lineArray = [
                ...
                worksheet[`E${rowIndex}`]?.v || "", // Option1 column E
                worksheet[`F${rowIndex}`]?.v || "", // Option2 column F/G
                //worksheet[`G${rowIndex}`]?.v || "",  
