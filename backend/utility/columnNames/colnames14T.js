...
// Define directories
const sourcedir = "./sourceT";
const goodDir = "./goodT";
const badDir = "./badT";

// Create log and data files
const logFilePath = `./logcolnames14T_${datetime}.txt`;
const dataFilePath = `./colnames14T_${datetime}.tsv`;
...
async function main() {
    ...
    console.log(`You have ${totalElements} files in this directory`);
    fs.appendFileSync(logFilePath, `You have ${totalElements} files in this directory\n`);
    ...
    for (let i = 0; i < totalElements; i++) {
        ...
        if (/^\d{6}$/.test(orderDate)) {
            ...
            errorOut(`Fail: ${sourcename}. Incorrect Order date format`);
            await fs.move(sourcename, relocdir);
            continue;
        }
